    ...
    public function selectContent($page, $filter, $fields = NULL, $limits=null)
    {
        ...
        if($filter['alter']){
            foreach($filter['alter'] as $key => $val){
                $sql = "SELECT content.* FROM content, field_value where content.id = field_value.element_id AND field_value.value LIKE '%". $val ."%' AND content.page_id = '". $page ."' AND field_value.code = '". $key ."' order by content.id DESC {$limit}";
            }
        } else {
            $sql = "SELECT * FROM content ". $where ." order by id DESC {$limit}";
        }
